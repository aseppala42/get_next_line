...
/*   Updated: 2019/12/06 14:48:38 by aseppala         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line(const int fd, char **line)
{
	...
	*line = 0;
	while (1)
	{
		*line = ft_strjoindel(*line, ft_strndup(next[fd],
					ft_strchrlen(next[fd], '\n')));
		if (ft_strchr(next[fd], '\n'))
		{
			ft_strcpy(next[fd], next[fd] + ft_strchrlen(next[fd], '\n') + 1);
			return (1);
		}
